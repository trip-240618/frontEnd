...
import 'package:tripStory/app/api/historyApi.dart';
...
class HistoryState extends GetxController {
  ...
  ///선택된 앨범 리스트
  final selectAlbumIndex = 0.obs;

  /// 클릭한 앨범 리스트
  final RxList imgUrl = [].obs;

  /// img url 저장하기
  final selectedDate = ''.obs;

  /// 사진 등록 할 때 날짜 선택
  final RxList addTagList = [].obs;

  /// 태그 추가할 리스트
  final historyTotalLen = 0.obs;

  /// 여행 기록 총 길이
  final uploadingLoading = false.obs;
  ...
  /// 여행 총 리스트
  final RxMap historyDetailList = {}.obs;

  /// 여행방 상세보기 리스트
  final RxList historyComment = [].obs;

  /// 댓글 리스트
  final RxList tagAllList = [].obs;

  /// 태그 전체 리스트
  final RxList tagSearchList = [].obs;

  /// 검색한 태그 리스트
  final RxList tagFilterColor = [].obs;

  /// 태그에서 컬러로 필터하고 싶을 때 리스트
  final RxList searchList = [].obs;

  /// 검색한 리스트들
  final RxList selectedTagList = [].obs;
  ...
  Future<Map<String, dynamic>> historyFileUpload(List files, BuildContext context) async {
    ...
    for (int i = 0; i < data['preSignedUrls'].length; i++) {
      ...
      await precacheImage(
          CachedNetworkImageProvider('${Uri.parse(data['preSignedUrls'][i].toString().split('?')[0])}'), context);
    }
