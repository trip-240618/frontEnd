...
class JPlanState extends GetxController {
  ...
  /// 수정 권한 버튼
  final RxList flightList = [].obs;

  /// 항공권 저장하는 리스트

  final selectedDate = ''.obs;

  /// 선택된 날짜를 저장하는 값 ex)항공권, 일정 추가 시 선택된 날짜
  ...
  /// jplan
  final RxList jPlanList = [].obs;

  /// jplan data 리스트
  final RxMap selectJplan = {}.obs;

  /// 수정 할 때 선택된 jplan 리스트
  final RxList editPlanJList = [].obs;

  /// 수정할 때 리스트
  final RxMap firstSwapList = {}.obs;
  ...
  /// 수정 할 때 날짜 변수
  /// planB
  final planBSelectedIdx = 0.obs;

  /// planB선택된 날짜 인덱스
  final planBSelectedDate = ''.obs;

  /// planB 선택된 날짜를 저장하는 값
  final RxList planBJList = [].obs;

  /// plan B j data 리스트
  final RxMap selectPlanBJList = {}.obs;

  /// 수정 할 때 선택된 plan B j  리스트
  /// planB add
  final Rx<DateTime> planBAddSelectedDateTime = DateTime.now().obs;
  final planBAddDate = ''.obs;
